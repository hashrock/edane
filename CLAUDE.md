...
↑ / ↓ も同じ理由で、**選択モードだけがレイアウトで変わる**。canvasの選択モードは兄弟を優先し（`moveUpSiblingFirst` / `moveDownSiblingFirst`）、兄弟が尽きたらフラット順にフォールバックして枝の外へ抜ける。不変条件が対象とする**編集モード**では canvas / outline とも `moveUp` / `moveDown`＝フラット順。**どちらのモードでも ↑ / ↓ が行き止まりになってはいけない**（編集モードならキャレットの閉じ込め、選択モードなら枝の末尾で操作不能になる）。移動先がないのはツリー全体の先頭/末尾ノードだけ。
