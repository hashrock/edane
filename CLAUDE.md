...
↑ / ↓ も同じ理由で、**選択モードだけがレイアウトで変わる**。canvasの選択モードは `moveUpSiblingFirst` / `moveDownSiblingFirst`＝兄弟を辿り、尽きたら枝の外へ出る（↑は親へ、↓はサブツリーを飛び越えて次のノードへ）。**子には決して降りない — 階層を降りるのは → の仕事**。例外はobjectカードだけで、行をカード内に描く＝視覚的に縦一列だから↓でカードの中へ入る（折りたたみ中は入らない、行の子孫にも入らない）。不変条件が対象とする**編集モード**では canvas / outline とも `moveUp` / `moveDown`＝フラット順。

**↑ / ↓ が行き止まりになってはいけない**（編集モードならキャレットの閉じ込め、選択モードなら枝の末尾で操作不能）。ただし「移動先が木構造から決まる」ことのほうが優先で、**同じ操作の意味が木の位置によって変わってはいけない**。かつて↓のフォールバックをフラット順の隣にしていたときは、「自分が親の最後の子か」というユーザーに見えない条件で↓が子に降りたり降りなかったりしていた。行き止まりを避けるためにフォールバックを足すときは、その移動先が木のどこでも同じ規則で決まるか確かめること。
