...
かつて↓のフォールバックをフラット順の隣にしていたときは、「自分が親の最後の子か」というユーザーに見えない条件で↓が子に降りたり降りなかったりしていた。**行き止まりを避けるためにフォールバックや記憶を足すときは、その移動先が木のどこでも同じ規則で決まるか確かめること。** objectカードの↓を「中を見たかどうか」で変えないのも同じ理由（←でタイトルに戻って↓すると再びカードに入るのは意図した挙動。カードを抜けるには行を辿る）。
