...
## 修正（`app/components/useNoteEditor.ts`）と再検証

両方を修正し、モデルにも `SpecFixed` として反映して `BaselineConsistent` が成立する
ことを確認した（`AutosaveFixed.cfg` → **No error**、全不変条件成立）。

- **① 版ガード**: 保存ごとに単調増加する `saveSeqRef` を採番し、成功時は
  `seq > ackedSeqRef` の時だけ baseline を更新（`lastSaved := Max(lastSaved, v)` 相当）。
  古い保存が後に完了しても baseline を巻き戻さない。
- **② 失敗時リトライ**: デバウンス効果で保存失敗時に指数バックオフ（1.5s→…→15s上限）で
  再アーム。成功またはモデル変化で停止。

モデル側は `SaveOkGuarded`（`lastSaved' = Max(lastSaved, v)`）と
`SaveFailRetry`（`timer' = "armed"`）として符号化。修正前は違反していた
`BaselineConsistent` が修正後は全到達状態で成立する。
