...
> **第8ラウンド（TLA+・2026-07）→ [`round8/`](./round8/)**
> ノートのライフサイクル（ロード→ログイン状態解決→描画→編集→デバウンス自動保存＋
> ナビゲーションガード、`useNoteEditor.ts`）を状態遷移システムとして検査。
> **成立**: ゲストは書き込みしない・編集を持ったまま黙って離脱しない・クライアントは
> 過少報告しない（＝**データ損失なし**）、保存失敗が無ければ編集はいずれサーバへ届く。
> **発見（綻び2件）**: ①自動保存失敗時に自動リトライされない（次編集/ナビ頼み）、
> ②編集が in-flight 保存中に入ると保存が並行し、レスポンス順序逆転で `lastSavedContentRef`
> が後退→偽「未保存」・冗長な再保存（版ガードが無い）。いずれも safety は保たれ UX 上の綻び。
> **両件とも修正済み**（保存にシーケンス番号を採番する版ガード＋失敗時のバックオフ再アーム）。
> モデルにも `SpecFixed` として反映し `BaselineConsistent` が成立することを再検証。
> 詳細 → [`round8/README.md`](./round8/README.md)。
