...
- 反例をドメイン用語で説明したもの → [`FINDINGS.md`](./FINDINGS.md)
- 反例を実コードで再現するテスト → [`repro/reproduce.test.ts`](./repro/reproduce.test.ts)（4件パス）
...
### 実コードでの反例再現
```bash
npx vitest run --config formal/repro/vitest.config.ts   # 4 tests pass
```
